...
try:
    ...
    # ── V12: 从 vars.yaml 读取 TESSERACT_PATH（可配置） ──
    _tesseract_cmd = None
    _vars_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "vars.yaml")
    if os.path.isfile(_vars_path):
        try:
            with open(_vars_path, 'r', encoding='utf-8') as _f:
                for _line in _f:
                    _m = re.match(r'^TESSERACT_PATH:\s*"?(.+?)"?\s*$', _line.strip())
                    if _m:
    ...
    # 如果 vars.yaml 未配置或文件不存在，回退到默认候选路径
    ...
    if _tesseract_cmd:
        ...
        print(f"  [OCR] Tesseract 路径: {_tesseract_cmd}")
    else:
        print("  [OCR] 未找到 Tesseract，OCR 功能不可用")
        HAS_PYTESSERACT = False
except ImportError:
    HAS_PYTESSERACT = False

###############################################################################
#  V12: 图像模糊匹配引擎（ImageMatcher）
#  用于 CDP/OCR 均失败的场景，通过模板匹配找到元素坐标并点击。
#  依赖: opencv-python (推荐) 或 pillow + numpy
###############################################################################
...
class ImageMatcher:
    """
    图像模糊匹配引擎。
    使用采集时的元素截图作为模板，在当前页面截图中进行模板匹配，
    返回匹配度最高的位置坐标。

    匹配算法:
      - OpenCV 可用: TM_CCOEFF_NORMED (最佳)
      - 仅 PIL: 像素级 diff 匹配（回退）
    """

    def __init__(self, page, event_bus: EventBus):
        self.page = page
        self.event_bus = event_bus
        self._available = HAS_CV2 or HAS_PIL_NP
        self._threshold = 0.6  # 匹配阈值，低于此值认为未找到
    ...
    def set_threshold(self, t: float):
        """设置匹配阈值 (0.0-1.0)，越低越模糊"""
        self._threshold = max(0.1, min(1.0, t))

    async def find_element(self, template_path: str) -> Optional[dict]:
        """
        在页面中查找与模板截图像匹配的元素。
        
        Args:
            template_path: 采集时保存的元素截图路径
            
        Returns:
            {"x": x, "y": y, "width": w, "height": h, "confidence": score}
            或 None（未找到）
        """
        ...
        try:
            ...
            page_img = Image.open(io.BytesIO(screenshot_bytes)).convert("RGB")
            template_img = Image.open(template_path).convert("RGB")

            if HAS_CV2:
                return self._match_cv2(page_img, template_img)
            else:
                return self._match_pil(page_img, template_img)
        except Exception as e:
            await self.event_bus.emit("img_match.error", msg=str(e))
            return None

    def _match_cv2(self, page_img, template_img) -> Optional[dict]:
        """OpenCV 模板匹配（推荐）"""
        page_cv = cv2.cvtColor(np.array(page_img), cv2.COLOR_RGB2GRAY)
        tpl_cv = cv2.cvtColor(np.array(template_img), cv2.COLOR_RGB2GRAY)
        ...
        for scale in scales:
            if scale != 1.0:
                ...
                if new_w < 10 or new_h < 10:
                    continue
                resized = cv2.resize(tpl_cv, (new_w, new_h))
            else:
                resized = tpl_cv

            if resized.shape[0] > page_cv.shape[0] or resized.shape[1] > page_cv.shape[1]:
                continue

            result = cv2.matchTemplate(page_cv, resized, cv2.TM_CCOEFF_NORMED)
    ...
    def _match_pil(self, page_img, template_img) -> Optional[dict]:
        """PIL + numpy 基本匹配（回退方案）"""
        pw, ph = page_img.size
        ...
        page_arr = np.array(page_img, dtype=np.int32)
        tpl_arr = np.array(template_img, dtype=np.int32)

        best_score = -1
        best_pos = None

        # 步进采样加速
        step = max(1, min(tw, th) // 20)
        for y in range(0, ph - th + 1, step):
            for x in range(0, pw - tw + 1, step):
                region = page_arr[y:y+th, x:x+tw]
                # 归一化平方差匹配
                diff = np.mean((region - tpl_arr) ** 2)
                # 转成相似度分数 (0-1)
                score = 1.0 / (1.0 + diff / 10000.0)
                if score > best_score:
                    best_score = score
                    best_pos = (x, y)

        if best_score >= self._threshold * 0.8:
            return {
                "x": int(best_pos[0]),
                "y": int(best_pos[1]),
                "width": tw,
...
class StepRunner:
    ...
    def __init__(self, page, cdp: CDPEngine, event_bus: EventBus,
        ...
        # V12: 图像匹配引擎（OCR 前一层兜底）
        self.img_matcher = ImageMatcher(page, event_bus) if (HAS_CV2 or HAS_PIL_NP) else None
        self.event_bus = event_bus
    ...
    async def _locate(self, strategies: list, max_retries=5, retry_delay=1.0,
        ...
        # ── 3. V12: 图像模糊匹配（如果元素有截图模板） ──
        if self.img_matcher and self.img_matcher.available:
            screenshot_file = None
            # 从策略中找 screenshot 文件名
            for s in strategies:
                if isinstance(s, dict) and s.get("type") == "screenshot":
                    screenshot_file = s.get("value")
                    break
            if screenshot_file and os.path.isfile(screenshot_file):
                print(f"      🖼️  启动图像模板匹配...")
                match = await self.img_matcher.find_element(screenshot_file)
                if match and match.get("confidence", 0) >= self.img_matcher._threshold:
                    cx = match["x"] + match["width"] // 2
                    cy = match["y"] + match["height"] // 2
                    await self.page.mouse.click(cx, cy)
                    conf = int(match["confidence"] * 100)
                    return ("img_found", match)
    ...
    async def _execute_action(self, loc, action: dict):
        ...
        # CDP/OCR/图像 找到的（非 locate_parent）
        if isinstance(loc, tuple):
            ...
            if loc[0] == "img_found":
                deep = loc[1]
                cx = deep["x"] + deep["width"] // 2
                cy = deep["y"] + deep["height"] // 2
                await self.page.mouse.click(cx, cy)
                conf = int(deep.get("confidence", 0) * 100)
                return True, f"图像模板匹配→已点击(相似度{conf}%)"
