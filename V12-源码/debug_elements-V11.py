...
class ElementCollector:
    ...
    def _get_full_picker_script(self) -> str:
        ...
        (async () => {
            ...
            window.__pickerInitInterval = setInterval(() => { if (document.body) initUI(); }, 1000);
            ...
            function onClick(e) {
                if (window.__pickerExited) return;
            ...
            window.__pickerExit = async function() {
                ...
                document.removeEventListener('click', onClick, true);
                document.removeEventListener('mousemove', onMouseMove, true);
                // 移除键盘监听（需要用具名函数，但匿名不行 → 直接替换为无操作）
                // 移除注入的 DOM 元素
                if (panel) { panel.remove(); panel = null; }
                if (fallbackBtn) { fallbackBtn.remove(); fallbackBtn = null; }
                if (hoverHighlight) { hoverHighlight.remove(); hoverHighlight = null; }
                const styleEl = document.getElementById('__pickerStyle');
                if (styleEl) styleEl.remove();
                const initInterval = window.__pickerInitInterval;
    ...
    async def _py_sync_state(self, action: str, payload=None):
        ...
        if action == "GET":
            ...
            try:
                rect = payload.get("selectors", {}).get("rect", {})
                if self._active_page and rect and rect.get("width", 0) > 0:
                    self._screenshot_counter += 1
                    stem = datetime.now().strftime("%H%M%S")
                    fname = f"el_{self._screenshot_counter:03d}_{stem}.png"
                    screenshot_path = os.path.join(SCREENSHOTS_DIR, fname)
                    # 截取全屏，然后裁剪到元素区域
                    await self._active_page.screenshot(
                    ...
                    payload["screenshot"] = fname
            except Exception as e:
                print(f"    [WARN] 元素截图失败: {e}")
