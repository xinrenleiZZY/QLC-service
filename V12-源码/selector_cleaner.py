...
DYNAMIC_ID_PATTERNS = [
    r'^el-id-\d+',
    r'^el-popper-container-\d+',     # Element UI 弹出容器，数字后缀动态变化
    r'^el-dialog__body-\d+',         # 对话框体
    r'^el-overlay-\d+',             # 遮罩层
    r':',
    r'^\d+$',
    r'^[0-9a-f]{8}-',
...
def _extract_stable_id_prefix(id_str: str) -> str:
    """
    从动态 ID 中提取稳定前缀。
    如 'el-popper-container-5062' → 'el-popper-container'
    如 'el-overlay-123' → 'el-overlay'
    """
    # 匹配末尾的 -数字 后缀
    m = re.match(r'^(.+?)-?\d+$', id_str)
    if m:
        candidate = m.group(1)
        # 确保提取的前缀最少有 3 个字符，避免误伤短 ID
        if len(candidate) >= 3:
            return candidate
    return ""
...
def build_anchor_strategies(ancestors: list, self_strategies: List[dict]) -> List[dict]:
    ...
    for ancestor in ancestors:
        ...
        elif ancestor.get("id") and is_dynamic_id(ancestor["id"]):
            # 动态 ID 但可能有稳定前缀 → 用 CSS 前缀匹配 [id^="xxx"]
            prefix = _extract_stable_id_prefix(ancestor["id"])
            if prefix:
                anchor_type = "css"
                anchor_value = f'[id^="{prefix}"]'
...
def clean_file(input_path: str, output_path: str = None, inplace: bool = False):
    ...
    if inplace:
        ...
        # V12: 统一输出到 docs/cleaned/ 目录
        base, ext = os.path.splitext(os.path.basename(input_path))
        # 检测项目根目录
        PROJECT_ROOT = os.path.dirname(os.path.abspath(__file__))
        docs_cleaned = os.path.join(PROJECT_ROOT, "docs", "cleaned")
        os.makedirs(docs_cleaned, exist_ok=True)
        output_path = f"{docs_cleaned}/{base}_cleaned{ext}"
