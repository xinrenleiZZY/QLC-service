"""
领星(Lingxing) 页面元素调试工具 (分离版：浏览器持久运行 + 工具按需连接)
用法：
  第一步：python debug_elements-V9.py --start-browser  # 启动持久浏览器
  第二步：python debug_elements-V9.py --attach         # 连接并注入拾取器
  第三步：使用完毕后，按 F4 或 Ctrl+C 退出工具（浏览器保持运行）
  第四步：python debug_elements-V9.py --close-browser  # 关闭浏览器
"""
...
(async () => {
    ...
    function initUI() {
        if (!document.getElementById('__pickerPanel')) {
            ...
            panel.innerHTML = `
                <div style="padding:8px 12px;background:#2d2d2d;border-bottom:1px solid #333;display:flex;justify-content:space-between;align-items:center;cursor:move; user-select:none;" id="__pickerHeader">
                    <span style="color:#569cd6;font-weight:bold;">🔍 元素拾取器</span>
                ...
                <div style="padding:6px 12px;background:#252526;border-bottom:1px solid #333;display:flex;align-items:center;gap:6px;">
                    ...
                    <button id="__exitToolBtn" style="background:#ce9178;color:#1e1e1e;border:none;padding:4px 12px;border-radius:4px;cursor:pointer;font-size:11px;font-weight:bold;">⏹ 退出工具</button>
                </div>
                ...
                <div style="padding:6px 12px;border-top:1px solid #333;font-size:11px;color:#888;display:flex;justify-content:space-between;align-items:center;">
                    <span>F1:拾取 | F2:高亮 | F3:命名 | F4:退出工具 | Ctrl+S:保存</span>
            ...
            const exitToolBtn = document.getElementById('__exitToolBtn');
            ...
            if (exitToolBtn) {
                exitToolBtn.onclick = function(e) { e.stopPropagation(); window.__pickerExitTool(); };
            }
        ...
        if (!document.getElementById('__pickerFallbackBtn')) {
            ...
            fallbackBtn.innerHTML = '🔍 开启拾取(F1) <span style="font-size:10px;color:#888;margin-left:6px;font-weight:normal;">右键退出工具</span>';
            ...
            fallbackBtn.oncontextmenu = function(e) { e.preventDefault(); e.stopPropagation(); window.__pickerExitTool(); };
    ...
    window.__pickerExitTool = async function() {
    ...
    window.addEventListener('keydown', function(e) {
        ...
        else if (e.key === 'F4') { e.preventDefault(); e.stopPropagation(); window.__pickerExitTool(); }
...
#  浏览器持久化服务
# ------------------------------------------------------------
BROWSER_PID_FILE = "browser_pid.txt"
CDP_PORT_FILE = "cdp_port.txt"
USER_DATA_DIR = os.path.join(os.path.dirname(__file__), "playwright_profile")


def start_persistent_browser():
    """启动一个持久化的浏览器实例（通过 Playwright 的 launch_persistent_context）"""
    print("正在启动持久化浏览器...")
    
    async def _start():
        p = await async_playwright().start()
        
        # 使用持久化上下文，这样 cookies、登录状态都会保留
        context = await p.chromium.launch_persistent_context(
            user_data_dir=USER_DATA_DIR,
            channel="chrome",
            headless=False,
            no_viewport=True,
            args=["--start-maximized", "--remote-debugging-port=9222"],  # 开启远程调试端口
            locale="zh-CN",
        )
        ...
        print(f"\n[浏览器已启动] CDP 端口: {cdp_port}")
        print(f"[持久化目录] {USER_DATA_DIR}")
        print(f"\n现在可以运行: python {sys.argv[0]} --attach 来连接拾取器")
...
def attach_to_browser():
    ...
    if not os.path.exists(CDP_PORT_FILE):
        print("错误：未找到运行的浏览器实例。请先运行:")
        print(f"  python {sys.argv[0]} --start-browser")
        sys.exit(1)
    ...
    async def _attach():
        ...
        for context in contexts:
            ...
            pages = context.pages
            for page in pages:
                await page.evaluate(ELEMENT_PICKER_JS)
            
            # 为未来的新页面自动注入
            await context.add_init_script(script=ELEMENT_PICKER_JS)
        
        print(f"\n[已连接] 已注入元素拾取器到 {len(browser.contexts)} 个上下文")
        print("\n🎯 使用说明:")
        print("  - 在任意已打开的页面中，应该能看到右下角的拾取按钮")
        print("  - 按 F1 激活拾取模式，点击元素即可记录")
        print("  - 按 F4 退出工具（浏览器保持运行）")
        print("  - Ctrl+S 保存采集数据到 JSON 文件")
        print("\n按 Ctrl+C 退出工具（浏览器保持运行）...\n")
...
#  兼容旧模式（一键启动并自动关闭）
# ------------------------------------------------------------
async def debug_mode_legacy(url: str = None):
    """旧模式：启动浏览器并在退出时关闭（保留兼容性）"""
    p = await async_playwright().start()
    user_data_dir = os.path.join(os.path.dirname(__file__), "playwright_profile")
    context = await p.chromium.launch_persistent_context(
        user_data_dir=user_data_dir,
...
def main():
    parser = argparse.ArgumentParser(description="领星页面元素调试工具 - 浏览器持久化版")
    parser.add_argument("--start-browser", action="store_true", help="启动持久化浏览器（独立运行）")
    parser.add_argument("--attach", action="store_true", help="连接到已运行的浏览器并注入拾取器")
    parser.add_argument("--close-browser", action="store_true", help="关闭持久化浏览器")
    parser.add_argument("--url", help="指定要调试的页面URL（仅旧模式）")
    parser.add_argument("--legacy", action="store_true", help="使用旧模式（退出时关闭浏览器）")
    ...
    if args.start_browser:
        ...
        close_browser()
    elif args.legacy or args.url:
        # 兼容旧模式
        asyncio.run(debug_mode_legacy(args.url))
    else:
        # 默认显示帮助
        print(__doc__)
        print("\n请选择运行模式:")
        print(f"  1. 启动浏览器:     python {sys.argv[0]} --start-browser")
        print(f"  2. 连接拾取器:     python {sys.argv[0]} --attach")
        print(f"  3. 关闭浏览器:     python {sys.argv[0]} --close-browser")
        print(f"  4. 旧模式(退出关): python {sys.argv[0]} --legacy")
